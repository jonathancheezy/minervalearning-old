...
### 🗄️ Before Every Task — Check Your Memory

Before executing any task, check relevant memory files. This is non-negotiable:

1. **New task on an existing project** → search `memory/` for project context
2. **Decisions or trade-offs** → check if MEMORY.md or daily logs have prior notes
3. **Repetitive or similar work** → look for patterns from previous sessions

This prevents: re-doing work, missing context, contradicting prior decisions, forgetting project state.

If memory search returns nothing relevant, proceed — but at least try.
